...
pdd_simulation_replicated <-
  function(rep, pars, age, model, metric) {
    require(tidyverse)
...
pdd_simulation_plot <- function(result, pars, age, model) {
  require(tidyverse)
  require(cowplot)
  ...
  if (model == "dsce1" | model == "dsde1") {
    ...
    title <-
      ggdraw() + draw_label(paste(model, ", ", "lambda = ", pars[1], ", ", "mu = ", pars[2], ", ", "K = ", pars[3], ", ", "age = ", age), fontface = 'bold')
  } else {
    plotPhi <-
      ggplot(result[[2]], aes(time, Phi, group = rep, color = rep)) + geom_line() +
      theme(legend.position = "none") + xlab("age")
    title <-
      ggdraw() + draw_label(paste(model, ", ", "lambda = ", pars[1], ", ", "mu = ", pars[2], ", ", "betaN = ", pars[3], ", ", "betaPhi = ", pars[4], ", ","age = ", age), fontface = 'bold')
  }
