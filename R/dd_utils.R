...
#' @param model the type of model used in simulation being plotted
#' @param K the phylogenetic carrying capacity used in simulation being plotted
#' @return a plot grid generated by cow_plot
#' @author Tianjian Qin
#' @keywords phylogenetics
#' @export pdd_simulation_plot
pdd_simulation_plot <- function(result, model, K = NULL) {
  require(tidyverse)
  require(cowplot)
  ...
  if (model == "dsce1" | model == "dsde1"){
    ...
    plotPhi <-
      ...
      theme(legend.position = "none") + xlab("age") + geom_hline(yintercept = K)
  } else {
    plotPhi <-
      ggplot(result[[2]], aes(time, Phi, group = rep, color = rep)) + geom_line() +
      theme(legend.position = "none") + xlab("age")
  ...
  return(cowplot::plot_grid(plotN, plotPhi, plotla, plotmu))
}
...
pdd_simulation_wrapper <- function(rep, pars, age, model, metric) {
  result <-
  ...
  pdd_simulation_plot(result, model = model, K = pars[3])
}