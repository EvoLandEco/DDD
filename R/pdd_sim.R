...
pdd_update_lamu <- function(lamu, Phi, pars, model) {
    if (model == "dsce1") {
        ...
        #linear PD dependence in speciation rate
        newla <- max(0, lamu[1, 1] * (1 - Phi / pars))
        newmu <- lamu[1, 2]
    } else if (model == "dsce2") {
        if (length(pars) != 3) {
            stop("missing parameter(s)")
...
pdd_sim <- function (pars,
                     ...
                     metric = "pd") {
    if (pars[1] < pars[2]) {
        stop('the function is designed for lambda_0 > mu_0')
    }
    ...
    if (length(pars) == 3 && model == "dsce1") {
        ...
        return(out)
    } else if (length(pars) == 3 && model != "dsce1") {
        stop("this model requires 3 parameters")
    }
    ...
    if (length(pars) == 4 && model == "dsce2") {
        ...
        return(out)
    } else if (length(pars) == 4 && model != "dsce2") {
        stop("this model requires 4 parameters")
    }
} 